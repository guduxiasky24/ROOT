...
<!--<font color="red">商品总价格请不要填写100，2300,5000这样的整数,整数价格计算出的云购码可能就为10000001</font>-->
					...
					<option value="">请选择</option>
							<s:iterator value="productTypeList" var="productTypes">
								<c:choose>
									...
									<c:otherwise>
									    <c:if test="${productTypes.ftypeId!=1000&&productTypes.ftypeId!=0}">
		...
		<tr>
			<td align="right"><font color="red">*</font>限购期数：</td>
			<td><input type="text" value="${product.periodLimit }" name="product.periodLimit" class="input-text" style="width:65px;padding-left:0px;text-align:center" onkeyup="value=value.replace(/\D/g,'')" name="yunjiage" />期 <font color="red">(默认为空  表示不限购期数，否则填写限购期数。)</font></td>
		</tr>
        
         <tr>
			<td align="right"><font color="red">*</font>是否参与返佣：</td>
			<td>
				<input type="radio" id="_product.commissionStatus_0" value="0" <c:if test="${product.commissionStatus==null || product.commissionStatus==0}">checked="checked" </c:if> name="product.commissionStatus"/><label for="_product.commissionStatus_0">是</label>
				<input type="radio" id="_product.commissionStatus_1" value="1" <c:if test="${product.commissionStatus!=null && product.commissionStatus==1}">checked="checked" </c:if> name="product.commissionStatus"/><label for="_product.commissionStatus_1">否</label>
			</td>
		</tr>
