        ...
        success: function(e) {
            ...
            if (e.length > 0) {
                for (var c = 0; c < e.length; c++) {
                    a += '<ul class="' + (e[c].status == 1 ? "liBg": "") + '">';
                    a += '<li class="pad">' + e[c].province + "," + e[c].city + "," + e[c].district + "," + e[c].address + "</li>";
                    a += '<li class="wid55">' + e[c].zipCode + "</li>";
                    a += '<li class="wid70">' + e[c].consignee + "</li>";
                    a += '<li class="wid110">' + e[c].phone + "</li>";
                    if (e[c].status == 1) {
                        a += '<li class="wid80 orange">\u9ed8\u8ba4\u5730\u5740</li>'
                    } else {
                        a += '<li class="wid80 lightBlue"><a class="blue" href="javascript:;" onclick="javascript:addressSetDefault(' + e[c].id + ');">\u8bbe\u4e3a\u9ed8\u8ba4\u5730\u5740</a></li>\n'
                    }
                    a += '<li class="wid70">' + (e[c].status == 1 ? "": ' <a href="javascript:;" onclick="javascript:addressDelete(' + e[c].id + ');" title="\u5220\u9664">\u5220\u9664</a>') + "</li>";
                    a += "</ul>"
                }
                if (f) {
...
function requireFieldValid(e, f, d) {
    return getTrimVal(e) == "" ? $("#" + f).attr("class", "orange").html(" <s></s> " + d) : $("#" + f).attr("class", "red").html(" *")
}
        ...
        $.ajax({
            ...
            success: function(g) {
                ...
                for (var d = 0; d < g.length; d++) {
                    f += '<option value="' + g[d][0] + '">' + g[d][1] + "</option>"
                }
                $("#sel_areaB").html(f);
            ...
            success: function(h) {
                ...
                if (h == "") {
                    ...
                    for (var f = 0; f < h.length; f++) {
                        g += '<option value="' + h[f][0] + '">' + h[f][1] + "</option>"
                    }
                }
                $("#sel_areaC").html(g)
            },
        ...
        $.ajax({
            ...
            success: function(k) {
                ...
                for (var g = 0; g < k.length; g++) {
                    h += '<option value="' + k[g][0] + '">' + k[g][1] + "</option>"
                }
                $("#sel_areaA").html(h)
            },
            error: function() {
...
function Consignee(w) {
    ...
    var A = function() {
        ...
        var a = true;
        if (b != "") {
            a = regularExpressionValid("txt_ship_mb", SHIP_MB_REGULAR_EXPRESSION, "ship_mb_valid_msg", SHIP_MB_FORMAT_ERROR)
        }
        return a
    };
    var s = function() {
...
function oString(c) {
    var b = [];
    for (var a in c) {
        c[a] = typeof c[a] == "string" ? '"' + c[a] + '"': (typeof c[a] == "object" ? oString(c[a]) : c[a]);
        b.push(a + ":" + c[a])
    }
    return "{" + b.join(",") + "}"
}
...
function addressDelete(c) {
    ...
    if (theAddressNum < 2) {
        ...
        $.PageDialog.showConfirm("\u60a8\u786e\u5b9a\u8981\u5220\u9664\u6b64\u914d\u9001\u5730\u5740\u5417\uff1f", d)
    }
    return false
};