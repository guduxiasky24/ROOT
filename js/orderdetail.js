$(document).ready(function() {
    ...
    function() {
        ...
        $.ajax({
            ...
            success: function(k) {
                ...
                for (var h = 0; h < k.length; h++) {
                    j += '<option value="' + k[h][0] + '">' + k[h][1] + "</option>"
                }
                $("#shipAreaA").html(j)
            },
            error: function() {
        ...
        $("#shipAreaB").change(function() {
            var j = $("#shipAreaB").val();
            var h = $("#shipAreaB option:selected").text();
            $.ajax({
                ...
                success: function(m) {
                    ...
                    if (m == "") {
                        l += '<option value="' + k + '">' + h + "</option>"
                    } else {
                        for (var k = 0; k < m.length; k++) {
                            l += '<option value="' + m[k][0] + '">' + m[k][1] + "</option>"
                        }
                    }
                    $("#shipAreaC").html(l)
                },
    ...
    $("#btn_consignee_save").unbind("click").bind("click",
    function() {
        ...
        $.ajax({
            ...
            data: "userJSON=" + encodeURIComponent(h),
            success: function(i) {
                if (i == "success") {
    ...
    $("#btn_consignee_cancle").unbind("click").bind("click",
    function() {
        g.hide();
        b.show();
    ...
    d.click(function() {
        var h = $('input:radio[name="selectAddrID"]:checked').val();
        if (h == null) {
            $.PageDialog('<dl class="sAltFail"><dd>\u8bf7\u9009\u62e9\u4e00\u4e2a\u6536\u8d27\u5730\u5740\uff01</dd></dl>', {
                W: 250,
                H: 50
            });
            return false;
            ...
            $.ajax({
                ...
                data: "userJSON=" + encodeURIComponent(i),
                success: function(j) {
                    if (j == "success") {
                        $.PageDialog('<dl class="sAltOK"><dd>\u64cd\u4f5c\u6210\u529f\uff01</dd></dl>', {
                            W: 160,
        ...
        $.ajax({
            ...
            success: function(i) {
                if (i == "success") {
                    ...
                    $.PageDialog('<dl class="sAltFail"><dd>\u786e\u8ba4\u6536\u8d27\u5931\u8d25\uff0c\u8bf7\u786e\u5b9a\u5df2\u7ecf\u53d1\u8d27</dd></dl>', {
                        W: 280,
                        H: 50
                    })
                }
    ...
    $("[name=btnReceived]").click(function() {
        $.PageDialog.showConfirm("\u60a8\u786e\u5b9a\u6536\u5230\u8d27\u4e86\u5417?", a)
    });
    function e(k) {
        var j = [];
        for (var h in k) {
            k[h] = typeof k[h] == "string" ? '"' + k[h] + '"': (typeof k[h] == "object" ? e(k[h]) : k[h]);
            j.push(h + ":" + k[h])
        }
        return "{" + j.join(",") + "}"
    }
